...
- Pour l’installation manuelle uniquement : un accès aux fichiers des applications locales, par exemple avec **Samba share**. L’installation via le dépôt GitHub ne nécessite pas Samba.
...
### Installation depuis le magasin Home Assistant

1. Préparez le broker MQTT et ses identifiants (voir « Préparer Mosquitto » ci-dessous).
2. Cliquez sur le bouton **Ajouter le dépôt à Home Assistant** en haut de ce README, puis confirmez dans votre instance.
3. Vous pouvez aussi ouvrir **Paramètres → Applications → Magasin d’applications → ⋮ → Dépôts**, ajouter `https://github.com/elmomotito/homeassistant-hellowatt`, puis actualiser le magasin si nécessaire.
4. Ouvrez **Hello Watt — collecte quotidienne** et cliquez sur **Installer**. L’image est construite sur votre machine ; la première installation peut prendre quelques minutes.
5. Dans **Configuration**, saisissez vos identifiants Hello Watt et MQTT. Laissez `home_id: ""` pour la détection automatique.
6. Pour le premier essai, réglez `run_on_start: true`, enregistrez puis démarrez l’application. Consultez ses journaux et la section « Première collecte ».
7. Après réussite, remettez `run_on_start: false` et `retry_once: ""`. Gardez l’application démarrée et son démarrage automatique activé pour la collecte à 8 h.

L’application apparaît dans le magasin après ajout de ce dépôt tiers. Elle n’est pas une application officielle de Home Assistant. Les nouvelles versions sont proposées par le magasin lorsque le numéro `version` du module augmente.

**Déjà installé comme application locale ?** L’installation depuis GitHub crée une application distincte, avec son propre dossier de données. Elle ne récupère pas automatiquement la base de l’application locale. Conservez celle-ci et sa sauvegarde ; ne faites pas tourner les deux collecteurs en même temps. Une migration de la base permet de conserver l’historique local, sinon la nouvelle installation devra relire les données encore disponibles chez Hello Watt.

### Installation locale — alternative

Les étapes suivantes servent à l’installation manuelle depuis les fichiers. Elles ne sont pas nécessaires si vous avez installé l’application depuis le dépôt GitHub.
